...
export class ZoomStatusBarEntry extends OwnedStatusBarEntry {
	public readonly onDidChangeScale: vscode.Event<{ scale: Scale }>;
	private readonly _onDidChangeScale = this._register(new vscode.EventEmitter<{ scale: Scale }>());

	public constructor() {
		super(
			"status.svgPreview.zoom",
			localize("zoomStatusBar.name", "Image Zoom"),
	...
	public show(owner: string, scale: Scale): void {
